...
@section('content')
    <div class="mx-auto max-w-5xl animate-fade-in" x-data="scanApp()" x-init="init()">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
            ...
            <div class="lg:col-span-7 space-y-4">
                ...
                <div id="camera-container"
                    class="camera-wrapper relative overflow-hidden rounded-2xl transition-all duration-300"
                    ...
                    <div x-show="!scanning && !cameraError" x-transition:enter="transition ease-out duration-300"
                        ...
                        <div
                            ...
                            <svg class="w-10 h-10" fill="none" viewBox="0 0 24 24" stroke-width="1.2"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <p class="mt-1.5 max-w-[260px] text-center text-xs text-gray-500 leading-relaxed">Arahkan kamera ke
                            barcode Code 128 yang ada di kartu siswa</p>
                        <div class="mt-6 grid grid-cols-2 gap-2 w-full max-w-[260px]">
                            ...
                            <div class="rounded-lg bg-white/[0.04] px-3 py-2.5 text-center">
                                <p class="text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-0.5">Posisi
                                </p>
                                <p class="text-xs text-gray-400">Tegak lurus</p>
                            </div>
                            <div class="rounded-lg bg-white/[0.04] px-3 py-2.5 text-center">
                                <p class="text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-0.5">Cahaya
                                </p>
                    ...
                    <div x-show="cameraError" x-transition:enter="transition ease-out duration-300"
                        ...
                        <p class="text-sm font-semibold text-red-300" x-text="cameraErrorTitle || 'Gagal Mengakses Kamera'">
                        </p>
                    ...
                    {{-- Scan Line --}}
                    <div x-show="scanning && !processing && !showingResult && !cameraError"
                        x-transition.opacity.duration.200ms class="pointer-events-none absolute inset-0 z-10">
                    ...
                    <div x-show="scanning && !cameraError" x-transition.opacity.duration.300ms
                        ...
                        <div
                            class="absolute top-4 left-4 w-7 h-7 border-l-2 border-t-2 rounded-tl-md border-indigo-400/80">
                        </div>
                        <div
                            class="absolute top-4 right-4 w-7 h-7 border-r-2 border-t-2 rounded-tr-md border-indigo-400/80">
                        </div>
                        <div
                            class="absolute bottom-4 left-4 w-7 h-7 border-l-2 border-b-2 rounded-bl-md border-indigo-400/80">
                        </div>
                        <div
                            class="absolute bottom-4 right-4 w-7 h-7 border-r-2 border-b-2 rounded-br-md border-indigo-400/80">
                        </div>
                    </div>
                    ...
                    <div x-show="showingResult" x-transition:enter="transition ease-out duration-300"
                        ...
                        <div
                            ...
                            <h3 class="text-base font-bold text-gray-800" x-text="resultData ? resultData.title : ''">
                            </h3>
                            <p class="text-sm font-bold text-gray-900 mt-1" x-text="resultData ? resultData.nama : ''">
                            </p>
            ...
            <div class="lg:col-span-5">
                <div class="lg:sticky lg:top-4 space-y-3">
                    ...
                    {{-- Terakhir Absen (Big Card) --}}
                    <div
                        class="last-scan-card rounded-2xl border border-indigo-100/80 bg-gradient-to-br from-indigo-50/80 via-white to-purple-50/40 p-5 shadow-sm">
                        <div class="flex items-center gap-2 mb-4">
                            ...
                            <h3 class="text-[11px] font-bold text-indigo-600 uppercase tracking-widest">Terakhir Absen</h3>
                        </div>

                        @if ($recentScans->count() > 0)
                            @php $last = $recentScans->first(); @endphp
                            <div id="last-scan-content" data-filled="1" data-nama="{{ $last->siswa->nama }}"
                                data-nipd="{{ $last->siswa->nipd ?? ($last->siswa->nis ?? '') }}"
                                data-kelas="{{ $last->siswa->kelas->nama ?? '-' }}" data-jam="{{ $last->jam_masuk }}"
                                data-jam-pulang="{{ $last->jam_pulang ?? '' }}"
                                data-type="{{ $last->jam_pulang ? 'pulang' : 'masuk' }}">
                                <div class="flex items-center gap-4">
                                    <div
                                        class="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-600 text-white text-lg font-bold shadow-lg shadow-indigo-500/25">
                                        {{ strtoupper(substr($last->siswa->nama, 0, 1)) }}
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <p class="text-[15px] font-bold text-gray-900 truncate">{{ $last->siswa->nama }}
                                        </p>
                                        <p class="text-xs text-gray-500 mt-0.5">
                                            {{ $last->siswa->nipd ?? ($last->siswa->nis ?? '') }} ·
                                            {{ $last->siswa->kelas->nama ?? '-' }}</p>
                                    </div>
                                    <div class="text-right shrink-0">
                                        <span
                                            class="inline-flex rounded-lg px-2.5 py-1 text-[10px] font-bold {{ $last->jam_pulang ? 'bg-blue-50 text-blue-700 ring-1 ring-blue-100' : 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-100' }}">
                                            {{ $last->jam_pulang ? 'Pulang' : 'Masuk' }}
                                        </span>
                                        <p class="text-sm font-mono font-semibold text-gray-700 mt-1.5">
                                            {{ $last->jam_masuk }}</p>
                                        @if ($last->jam_pulang)
                        ...
                        @else
                            <div id="last-scan-empty" class="flex flex-col items-center justify-center py-5 text-center">
                                <div
                                    class="flex h-11 w-11 items-center justify-center rounded-xl bg-indigo-50 text-indigo-300 mb-2.5">
                                ...
                                <p class="text-sm font-medium text-gray-400">Belum ada absensi</p>
                                <p class="text-[11px] text-gray-300 mt-0.5">Scan barcode untuk memulai</p>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="rounded-2xl border border-gray-100 bg-white shadow-sm overflow-hidden">
                        <div class="flex items-center justify-between border-b border-gray-50 px-5 py-3.5">
                            <div class="flex items-center gap-2">
                                ...
                                <span
                                    class="inline-flex items-center justify-center rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-bold text-gray-500"
                                    x-text="scanCount"></span>
                            </div>
                            <a href="{{ route('scanner.riwayat.index') }}"
                                class="text-xs font-semibold text-indigo-600 hover:text-indigo-700 transition-colors">Lihat
                                Semua →</a>
                        </div>
                        <div id="scan-list" class="divide-y divide-gray-50 max-h-[340px] overflow-y-auto">
                            @foreach ($recentScans->skip(1) as $absensi)
                                <div class="flex items-center gap-3.5 px-5 py-3 hover:bg-gray-50/70 transition-colors">
                                    <div
                                        class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-indigo-50 text-[11px] font-bold text-indigo-600">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <p class="text-[11px] text-gray-400 mt-0.5">
                                            {{ $absensi->siswa->nipd ?? $absensi->siswa->nis }}
                                            · {{ $absensi->siswa->kelas->nama ?? '-' }}</p>
                                    </div>
                                    <div class="text-right shrink-0">
                                        <p class="text-xs font-mono font-medium text-gray-600">{{ $absensi->jam_masuk }}
                                        </p>
                            ...
                            @if ($recentScans->count() <= 1)
                                <div class="flex flex-col items-center justify-center py-10 text-center">
                                    <p class="text-xs text-gray-300">Belum ada riwayat lain</p>
                                </div>
                            @endif
                        </div>
                    </div>
            ...
            function scanApp() {
                return {
                    ...
                    addToList(result) {
                        ...
                        const cardContent = document.getElementById('last-scan-content');
                        const cardEmpty = document.getElementById('last-scan-empty');
                        if (!list || !result.data) return;
                        const d = result.data;
                        const masuk = result.type === 'masuk';

                        // Hapus empty state di list
                        const emptyList = list.querySelector('.flex-col');
                        if (emptyList) emptyList.remove();

                        // Sembunyikan empty state di card
                        if (cardEmpty) cardEmpty.style.display = 'none';

                        // Pindahkan isi card lama ke list (jika card sudah terisi)
                        if (cardContent && cardContent.dataset.filled === '1') {
                            const prevNama = cardContent.dataset.nama || '?';
                            const prevNipd = cardContent.dataset.nipd || '';
                            const prevKelas = cardContent.dataset.kelas || '-';
                            const prevJam = cardContent.dataset.jam || '-';
                            const prevJamPulang = cardContent.dataset.jamPulang || '';
                            const prevType = cardContent.dataset.type || 'masuk';
                            const prevDiv = document.createElement('div');
                            prevDiv.className =
                                'flex items-center gap-3.5 px-5 py-3 hover:bg-gray-50/70 transition-colors anim-fade-up';
                            prevDiv.innerHTML = `
                                <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-indigo-50 text-[11px] font-bold text-indigo-600">${prevNama[0].toUpperCase()}</div>
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-semibold text-gray-800 truncate">${prevNama}</p>
                                    <p class="text-[11px] text-gray-400 mt-0.5">${prevNipd} · ${prevKelas}</p>
                                </div>
                                <div class="text-right shrink-0">
                                    <p class="text-xs font-mono font-medium text-gray-600">${prevJam}</p>
                                    ${prevJamPulang ? '<p class="text-[10px] font-mono text-gray-400 mt-0.5">↗ ' + prevJamPulang + '</p>' : ''}
                                    <span class="inline-flex rounded-lg px-2 py-0.5 text-[10px] font-semibold mt-0.5 ${prevType === 'masuk' ? 'bg-emerald-50 text-emerald-700' : 'bg-blue-50 text-blue-700'}">${prevType === 'masuk' ? 'Masuk' : 'Pulang'}</span>
                                </div>`;
                            list.insertAdjacentElement('afterbegin', prevDiv);
                        }

                        // Update card dengan data terbaru
                        if (cardContent) {
                            cardContent.dataset.filled = '1';
                            cardContent.dataset.nama = d.nama || '';
                            cardContent.dataset.nipd = d.nipd || d.nis || '';
                            cardContent.dataset.kelas = d.kelas || '';
                            cardContent.dataset.jam = masuk ? (d.time || '-') : (d.jam_masuk || '-');
                            cardContent.dataset.jamPulang = masuk ? '' : (d.jam_pulang || '');
                            cardContent.dataset.type = result.type || 'masuk';
                            cardContent.innerHTML = `
                                <div class="flex items-center gap-4 anim-fade-up">
                                    <div class="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-600 text-white text-lg font-bold shadow-lg shadow-indigo-500/25">${(d.nama || '?')[0].toUpperCase()}</div>
                                    <div class="flex-1 min-w-0">
                                        <p class="text-[15px] font-bold text-gray-900 truncate">${d.nama || '-'}</p>
                                        <p class="text-xs text-gray-500 mt-0.5">${d.nipd || d.nis || ''} · ${d.kelas || '-'}</p>
                                    </div>
                                    <div class="text-right shrink-0">
                                        <span class="inline-flex rounded-lg px-2.5 py-1 text-[10px] font-bold ${masuk ? 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-100' : 'bg-blue-50 text-blue-700 ring-1 ring-blue-100'}">${masuk ? 'Masuk' : 'Pulang'}</span>
                                        <p class="text-sm font-mono font-semibold text-gray-700 mt-1.5">${masuk ? (d.time || '-') : (d.jam_masuk || '-')}</p>
                                        ${!masuk && d.jam_pulang ? '<p class="text-[11px] font-mono text-gray-400 mt-0.5">↗ ' + d.jam_pulang + '</p>' : ''}
                                    </div>
                                </div>`;
                        }
                    },

...
@section('title', 'Naik Kelas - Per Kelas')
    ...
    <div class="space-y-6">
        ...
        <div>
            <div class="flex items-center gap-2 text-sm text-gray-500">
                ...
                <span class="text-gray-900 font-medium">Per Kelas</span>
            </div>
            <h1 class="text-2xl font-bold text-gray-900 mt-1">Naik Kelas Per Kelas</h1>
        </div>
        ...
        <div class="grid gap-6 lg:grid-cols-3">
            ...
            <div class="lg:col-span-1 space-y-4">
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5 space-y-4">
                    <h3 class="font-semibold text-gray-900">Pilih Kelas Asal</h3>

                    <form method="GET" action="{{ route('admin.naik-kelas.per-kelas') }}">
                        <div>
                            ...
                            <select name="kelas_asal_id"
                                ...
                                @foreach ($kelases as $k)
                                    <option value="{{ $k->id }}"
                                        {{ old('kelas_asal_id', $selectedKelas?->id) == $k->id ? 'selected' : '' }}>
                                        [{{ $k->tingkat }}] {{ $k->nama }} — {{ $k->jurusan->nama }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    @if ($selectedKelas)
                        <div class="bg-gray-50 rounded-lg p-3 text-sm text-gray-600">
                            <p><strong>[{{ $selectedKelas->tingkat }}] {{ $selectedKelas->nama }}</strong></p>
                            <p class="text-xs mt-1">{{ $selectedKelas->jurusan->nama }} · {{ $siswas->count() }} siswa
                                aktif</p>
                        </div>
                    @endif
                </div>
            ...
            <div class="lg:col-span-2">
                @if (!$selectedKelas)
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
                        ...
                        <p class="mt-3 text-gray-400">Pilih kelas asal untuk menampilkan daftar siswa</p>
                    </div>
                ...
                @else
                    <form method="POST" action="{{ route('admin.naik-kelas.per-kelas.store') }}" x-data="{ selectAll: false, showModal: false }"
                        x-ref="formPerKelas">
                        @csrf
                        <input type="hidden" name="kelas_asal_id" value="{{ $selectedKelas->id }}">
                        ...
                        <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                            ...
                            <!-- List -->
                            <div class="divide-y divide-gray-100 max-h-[450px] overflow-y-auto" x-ref="siswaList">
                                @foreach ($siswas as $index => $siswa)
                                    <label class="flex items-center gap-3 px-5 py-3 hover:bg-gray-50 cursor-pointer">
                                       ...
                                       selectAll = Array.from(boxes).every(cb => cb.checked);
                                   ">
                                        <span class="text-sm text-gray-400 w-6 shrink-0">{{ $index + 1 }}.</span>
                                        <div class="flex-1 min-w-0">
                                            <p class="text-sm font-medium text-gray-900 truncate">{{ $siswa->nama }}</p>
                                            <p class="text-xs text-gray-500">NIS:
                                                {{ $siswa->nis }}{{ $siswa->nipd ? ' · NIPD: ' . $siswa->nipd : '' }}
                                            </p>
                                        </div>
                                    </label>
                                @endforeach
                            ...
                            <div
                                ...
                                <div class="flex-1 w-full sm:w-auto">
                                    ...
                                    <select name="kelas_tujuan_id" required
                                        ...
                                        @foreach ($kelasTujuan->where('id', '!=', $selectedKelas->id)->groupBy('tahun_ajaran') as $ta => $items)
                            ...
                            <div class="fixed inset-0 flex items-center justify-center p-4">
                                <div class="bg-white rounded-xl shadow-xl max-w-sm w-full p-6" @click.stop>
                                    ...
                                    <div class="mt-6 flex gap-3">
                                        ...
                                        <button type="button" @click="$refs.formPerKelas.submit()"

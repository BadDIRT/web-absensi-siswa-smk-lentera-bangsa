...
@section('content')
    <div class="animate-fade-in">
        ...
        <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-sm font-semibold text-gray-800">Kelola Pengguna</p>
                    <p class="text-xs text-gray-400">{{ $users->total() }} pengguna</p>
                </div>
            </div>
            ...
            <table class="w-full text-left text-sm">
                <thead>
                    <tr class="border-b border-gray-100 bg-gray-50/50">
                        ...
                        <th class="px-5 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400">Username</th>
                        <th class="px-5 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400">Email</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($users as $u)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            <td class="px-5 py-3">
                                <a href="{{ route('admin.pengaturan.show', $u) }}"
                                    class="flex items-center gap-2.5 font-medium text-gray-800 hover:text-brand-600 transition-colors">
                                    <div
                                        class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-brand-50 text-xs font-bold text-brand-700">
                                        {{ strtoupper(substr($u->name, 0, 1)) }}
                                    </div>
                                    {{ $u->name }}
                                </a>
                            </td>
                            <td class="px-5 py-3 font-mono text-xs text-gray-600">{{ $u->username }}</td>
                            ...
                            <td class="px-5 py-3 text-right">
                                <div class="inline-flex items-center gap-0.5">
                                    ...
                                    @if ($u->id !== auth()->id())
                                        <button type="button"
                                            onclick="openDeleteModal('{{ $u->name }}', '{{ route('admin.pengaturan.destroy', $u) }}', '{{ csrf_token() }}')"
                                    ...
                                    @else
                                        <span class="inline-block w-[34px]" title="Akun sendiri"></span>
                                    @endif
                                </div>
                            </td>
                        </tr>
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 hidden items-center justify-center p-4"
        ...
        <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
            ...
            <h3 class="text-base font-semibold text-gray-900">Hapus Pengguna</h3>
            <p class="mt-1.5 text-sm text-gray-500 leading-relaxed">
                Yakin ingin menghapus pengguna <strong id="deleteItemName" class="text-gray-800"></strong>?
            </p>
